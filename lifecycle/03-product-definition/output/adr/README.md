...
These are produced in [Stage 3, part B](../../README.md). The folder is empty until you run it.
...
Whatever else you record, these three must be answerable from this folder, because later stages read them rather than asking you again:

| Question | Who reads the answer |
| --- | --- |
| What language and framework is this built in? | Stage 6, so it writes code in your stack |
| Where does the data live? | Stage 6 and Stage 9's runbook |
| What exact command runs the tests? | Stage 6 after every task, and Stage 8 at release |

The test command is the one people forget. Write it as something you could paste into a terminal — `pytest`, `npm test`, `go test ./...` — not as "run the unit tests".
