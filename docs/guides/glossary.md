...
| **Acceptance criteria** (often shortened to **AC**) | The checklist that decides whether a feature is done. "Done" means every box is ticked — not "it ran on my machine once". |
...
| **RPI** (research, plan, implement) | The three-step routine for writing code: first the AI investigates and writes down what it found, then it writes a plan, and only then does it write code. Each step is saved as a file, and you check each one before allowing the next. This is what stops the AI from confidently building the wrong thing. |
| **Gate** | A checkpoint you tick before moving on. If the previous step's file does not exist, you do not proceed. |
...
| --- | --- |
| `lifecycle/` | The nine stages. Each has `input/` (what it reads), `output/` (what it produces), and `prompt/` (the page telling you what to do). |
...
| `docs/guides/` | This glossary and the background reading. |
| `.copilot-tracking/` | Working notes the helpers save automatically as they research and plan. You rarely need to open it, but reviewers do. |
